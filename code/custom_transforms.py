...
class RGB2LAB(object):
    def __init__(self, ab_bins_dict, color_space='lab'):
        self.color_space = color_space
        self.ab_bins = ab_bins_dict
        self.num_bins = len(ab_bins_dict)
        self.nbrs = NearestNeighbors(n_neighbors=5, algorithm='ball_tree').fit(self.ab_bins)

    def __call__(self, sample):
        sample = np.asarray(sample, dtype=np.float32) / 255

        # Image read in by PIL follows RGB convention, therefore the conversion
        # is RGB2LAB
        ...
        z_truth_reshaped[indices,
                         np.arange(num_pixels).reshape(num_pixels, 1)] = norm.pdf(distances, scale=5)
